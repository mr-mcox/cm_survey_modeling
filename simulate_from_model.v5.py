...
for survey_seq in range(1,7):
    ...
    traces = dict()
    for q in qs:
        traces[q] = pd.read_csv('traces/v5_qc_{}_survey_seq_{}/chain-0.csv'.format(q, survey_seq))
    #Do runs
    counts = list()
    nets = list()
    ...
    for r in range(num_runs):
        ...
        for q in qs:
            ...
            for level in range(1, 8):
                mu = trace.get_value(rand_row, 'level_mu__{}'.format(level - 1))
                sigma = trace.get_value(rand_row, 'sigma__{}'.format(level - 1))
...
plt.savefig('./outputs/simulation_results_v5.png')
