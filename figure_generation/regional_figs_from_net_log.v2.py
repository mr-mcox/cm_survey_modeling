...
#Import traces
trace_files = [os.path.join('..','traces','net_log_v2_no_2015_survey_seq_{}'.format(s), 'chain-0.csv') for s in range(1,7)]
traces = {s: pd.read_csv(os.path.join('..','traces','net_log_v2_no_2015_survey_seq_{}'.format(s), 'chain-0.csv')) for s in range(1,7)}
...
#Add cohort
nets['cohort'] = pd.to_numeric(nets.survey_code.str.extract('^(\d{2})',expand=False)) + 2000
nets.ix[nets.Corps =='2nd year', 'cohort'] = nets.ix[nets.Corps =='2nd year', 'cohort'] - 1

def compute_next_resp(responses, alpha, beta, sigma):
    new_resp_mu = 1 / (1 + np.exp(beta * responses + alpha))
...
def simulate_seq(init, start_seq, end_seq):
    ...
    last_resp = np.array(init) * np.ones((trace_len,1))
    ...
    for seq in range(start_seq,end_seq + 1):
        ...
        sim_net.append(np.mean(last_resp, axis=1))
    return sim_net
...
def plot_projections(data, region_name):
    ...
    survey_seq_list = data.ix[data.Corps.notnull(),['survey_seq','survey_mod', 'Corps']].drop_duplicates()
    survey_seq_list['survey_label'] =  survey_seq_list.survey_mod + '-' + survey_seq_list.Corps
    ...
    plt.fill_between(np.arange(3) + 1, *qs, alpha=0.15,
                     color=c_pal[3])
    qs = mquantiles(sim_1y, [0.25, 0.75], axis=1).T
    ...
    plt.fill_between(np.arange(3) + 4, *qs, alpha=0.15,
                     color=c_pal[2], label='50% likelihood')
    qs = mquantiles(sim_2y, [0.25, 0.75], axis=1).T
    plt.fill_between(np.arange(3) + 4, *qs, alpha=0.35,
                     color=c_pal[2], label='95% likelihood')
    
    ax.xaxis.set_ticks([k for k in survey_seq_map['survey_mod'].keys()])
    labels = [survey_seq_map['survey_label'][i] for i in ax.get_xticks().tolist()]
    ...
    out_file = os.path.join('..','outputs','figures','projections_from_net_log_v2','Projection for {}.png'.format(region_name))
