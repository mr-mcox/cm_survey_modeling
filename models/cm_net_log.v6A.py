...
with pm.Model() as model:
    survey_mu = pm.Normal(
        'survey_mu', mu=transform_resp(0.4), sd=5, shape=survey_mtx.shape[1], testval=np.array([-1.83656424, -1.93631312, -2.19176981]))
    ...
    resp_mu = theano.dot(survey_mtx, survey_mu) + \
        delta_2y * is_2y + (survey_mtx * theano.dot(reg_mtx, reg_delta)).sum(axis=1)
