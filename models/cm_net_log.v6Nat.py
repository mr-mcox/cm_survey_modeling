...
with pm.Model() as log_model:
    ...
    # reg_delta_dist_sd = pm.Uniform('reg_delta_dist_sd', lower=0, upper=5)

    sigma = pm.HalfCauchy('sigma', beta=10)

    delta_2y = pm.Normal('delta_2y', mu=0, sd=3, testval=0)

    # reg_delta = pm.Normal(
    #     'reg_delta', mu=reg_delta_dist_mu, sd=reg_delta_dist_sd, testval=0, shape=(reg_mtx.shape[1], survey_mtx.shape[1]))

    prev = transform_resp(data.prev_response)
    resp = transform_resp(data.response)

    resp_mu = theano.dot(survey_mtx, survey_mu) + \
        delta_2y * is_2y
...
with open('../traces/net_log_v{}_columsn.json'.format(version), 'w') as json_file:
    json.dumps(col_headers, json_file)
