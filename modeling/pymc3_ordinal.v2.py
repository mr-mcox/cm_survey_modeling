...
test_values=[np.random.choice(7, 6000, p=[ 0.0010582 ,  0.01904762,  0.03915344,  0.02751323,  0.14285714, 0.48359788,  0.28677249])]


@theano.compile.ops.as_op(itypes=[t.dscalar, t.dscalar, t.dvector],otypes=[t.dvector])
def compute_category_p(mu, eff_sigma, thresh):
    p = [norm.cdf(thresh[i], mu, eff_sigma) -
                    norm.cdf(thresh[i-1], mu, eff_sigma) for i in range(1, len(thresh))]
    return np.array(p)
...
with pm.Model() as ordinal_model:
    ...
    category_p = compute_category_p(mu, sigma, f_thresh)
